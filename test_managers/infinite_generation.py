...
class InfiniteGenerationManager(BaseTestManager):
    ...
    def save_png_to_rs(self,input_file, recovery_shards):
        # 读取PNG文件内容
        ...
        output_file = parent_path + '/' + filename + '.rs'
        
        byte_msg=RSCodec(recovery_shards).encode(data)

        # 将原始数据和恢复数据写入输出文件
        with open(output_file, 'wb') as file:
            # for shard in binary_code:
            file.write(byte_msg)
        print("save rs file:" ,output_file)
    def save_meta_imgs(self, meta_img):
        ...
        print("pad image cost:",pad_time,"s")

        start_time = time.time()
        meta_img_np = (((meta_img.clamp(-1, 1).permute(0, 2, 3, 1) + 1) / 2).numpy() * 255).astype(np.uint8)
        meta_img_np_cost = time.time() - start_time
        print("clamp_permute_meta_img_np cost:",meta_img_np_cost,"s")
        ...
        for i in range(self.config.train_params.batch_size):
            ...
            save_path = os.path.join(self.save_root, str(global_id).zfill(6)+".tiff")
            print(save_path)
            tifffile.imsave(save_path, meta_img_np[i])
            # 保存 rs 格式
            self.save_png_to_rs(save_path,6)
            # plt.imsave(save_path, meta_img_np[i])
        save_cost = time.time() - start_time
        print("save image cost:",save_cost,"s")
